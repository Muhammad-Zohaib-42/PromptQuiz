  ...
  return (
    <form onSubmit={handleSubmit(submitHandler)} className="flex flex-col gap-3">
      <div className="flex flex-col gap-1 relative">
        <label className="relative required w-fit" htmlFor="name">
          Name{" "}
          <span className="text-red-400 absolute -top-0.5 -right-2">*</span>
        </label>
        <input
          className={`bg-zinc-800 text-zinc-50 placeholder:text-zinc-500 border border-zinc-600 focus:outline-none focus:ring ${errors.username ? "focus:ring-red-500" : "focus:ring-violet-500"} px-3 py-1 transition-all rounded-md`}
          type="text"
          id="name"
          {...register("username", {required: "username is required"})}
        />
        {errors.username && <span className="absolute top-0 right-0 text-red-400 text-sm">{errors.username.message}</span>}
      </div>
      <div className="flex flex-col gap-1 relative">
        ...
        <input
          ...
          type="email"
          id="email"
          {...register("email", {required: "Email is required",pattern: {value: /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i,message: "Invalid email address format"}})}
        />
      ...
      <div className="flex flex-col gap-1 relative">
        ...
        />
        <button type="button" className="absolute right-2 top-[58%] cursor-pointer" onClick={() => setIsPasswordVisible(!isPasswordVisible)}>
